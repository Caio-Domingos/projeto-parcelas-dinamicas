...
el_value_atual_init.textContent = radio_value.find(r => r.name === 'radio-1').value;
document.querySelectorAll('.price-selected').forEach(el => {
    el.addEventListener('change', function (event) {
        el_value_atual_init.textContent = radio_value.find(r => r.name === event.srcElement.id).value;
    });
});
/*
 ...
 */
function getValues() {
    const value = String(el_value_atual.textContent).split(' ')[1];
 ...
 * @returns vold
 */
function parcelas(value) {
    const valueNumber = parseFloat(value.replace(',', '.'));
    for (let index = 1; index <= 12; index++) {
        document.querySelector('#parcela-' + index).textContent = ((valueNumber / 12) *
            index)
            .toFixed(2)
            .replace('.', ',');
        if (index === 6) {
            document.querySelector('#parcela-' + index + '-amostra').textContent = ((valueNumber / 12) *
                index)
                .toFixed(2)
                .replace('.', ',');
        }
